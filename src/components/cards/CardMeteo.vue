  ...
  <div>
    <table class="border-collapse table-auto w-full text-sm rounded-xl">
      <thead>
        <tr>
          <td v-if="displayName === undefined" colspan="11" class="text-center">
            ...
            {{
              forecastCollection.position !== undefined
                ? forecastCollection.position.name
                : "chargement"
            }}
            ({{
              forecastCollection.position !== undefined
                ? forecastCollection.position.alti
                : "…"
            }}m)
          </td>
        ...
        <tr>
          <td colspan="11" class="text-center">
            <a href="https://meteo.fr/" target="_blank" rel="noopener">
              ...
              {{
                forecastCollection.updated_on !== undefined
                  ? getDate(forecastCollection.updated_on)
                  : "…"
              }}
            </a>
          </td>
        ...
        <template
          v-if="baliseFfvl !== undefined && baliseFfvl.balise !== undefined"
        >
          <tr>
            <td colspan="11" class="text-center text-blue-500">
              {{ $t("temps-reel") }}
              {{
                baliseFfvl.baliseName === place.properties.name
                  ? ""
          ...
          <tr>
            <td colspan="11" class="text-center text-blue-500">
              {{ $t("moy") }}:
              <b>{{ baliseFfvl.balise.vitesseVentMoy }}km/h</b> - Max:
              {{ baliseFfvl.balise.vitesseVentMax }}km/h - {{ $t("min") }}:
              {{ baliseFfvl.balise.vitesseVentMin }}km/h -
              <b>{{
              ...
              <svg
                :style="
                  getWindImg(Number(baliseFfvl.balise.directVentMoy)).style
                "
        ...
        <tr>
          ...
          <th scope="col" class="hidden md:inline-flex">
            {{ $t("humidite") }}
          </th>
          <th scope="col">
      ...
      <tbody v-if="forecastCollection">
        ...
        <template v-for="(detail, index) in forecastCollection.forecast">
          <tr
            class="d"
            :class="
              isDaylight(
          ...
          >
            ...
            >
              ...
              {{
                index != 0
                  ? getStartDay(
                      detail.dt,
                      forecastCollection.forecast[index - 1].dt
                    )
            ...
            >
              <pop-over-simple
                ...
                :text="
                  getWindAdequate(
                    place.properties.fly,
                    detail.wind.speed !== 0
                      ? Math.round(detail.wind.speed * 3.6).toString()
                      : 'v',
                    detail.wind.direction.toString() 
                  )
                "
                :title="$t('vent-admissible')"
                :left="false"
                ref="windPop"
              />
              <svg
...
import PopOverSimple from "../utilities/PopOverSimple.vue";
...
import type {
  ...
  DailyForecast,
  Weather12HOrWeather,
  Weather12HOrWeatherLong,
  RainOrSnow,
  Sun,
  Precipitation,
  Temp,
} from "../../types/ForecastCollection";
import type { BaliseData } from "../../plugins/BaliseFFVLHelper";
import {
  getBaliseData,
  baliseNull,
  getWindSector,
} from "../../plugins/BaliseFFVLHelper";
...
const nullDailyForecast = [
  {
    dt: 0,
    T: { min: 0 } as Temp,
    sun: { rise: 8, set: 18 } as Sun,
    precipitation: { "24h": 0 } as Precipitation,
    uv: 0,
  } as DailyForecast,
] as DailyForecast[];
const forecastCollection = ref<ForecastCollection>({
  daily_forecast: nullDailyForecast as DailyForecast[],
} as ForecastCollection);
const baliseFfvl = ref<BaliseData>({
  balise: baliseNull,
  baliseName: "",
  flyable: false,
} as BaliseData);
...
function getWindAdequate(
  flying: GeoJSON.FlyingPlaceProperties["fly"],
  currentWind: string = "v",
...
function showWeatherDetail(index: number) {
  weatherDetailClicked.value = weatherDetailClicked.value == index ? -1 : index;
  windDetailClicked.value = -1;
...
function getWeatherAtPlace(place: GeoJSON.FlyingPlace = props.place) {
  const src = `https://webservice.meteofrance.com/forecast?token=${API_TOKEN}&lat=${
    place.geometry.coordinates[1]
  }&lon=${place.geometry.coordinates[0]}&lang=${props.lang.substring(0, 2)}`;
...
function getWindImg(direction: number): {
  src: string;
